...
#include "src/mapping/sparse_pose_graph/proto/constraint_builder_options.pb.h"
//#include "src/sensor/compressed_point_cloud.h"
...
SparsePoseGraph2D::SparsePoseGraph2D(
...
std::vector<mapping::SubmapId> SparsePoseGraph2D::GrowSubmapTransformsAsNeeded(
...
void SparsePoseGraph2D::AddScan(
...
void SparsePoseGraph2D::AddWorkItem(const std::function<void()>& work_item) {
...
//void SparsePoseGraph2D::AddImuData(const int trajectory_id,
...
void SparsePoseGraph2D::AddOdometerData(
...
//void SparsePoseGraph2D::AddFixedFramePoseData(
//    const int trajectory_id,
//    const sensor::FixedFramePoseData& fixed_frame_pose_data) {
//  LOG(FATAL) << "Not yet implemented for 2D.";
//}

void SparsePoseGraph2D::ComputeConstraint(const mapping::NodeId& node_id,
                                        const mapping::SubmapId& submap_id) {
...
void SparsePoseGraph2D::ComputeConstraintsForOldScans(
    const mapping::SubmapId& submap_id) {
...
void SparsePoseGraph2D::ComputeConstraintsForScan(
    const int trajectory_id,
...
void SparsePoseGraph2D::HandleWorkQueue() {
...
void SparsePoseGraph2D::WaitForAllComputations() {
...
void SparsePoseGraph2D::FreezeTrajectory(const int trajectory_id) {
  common::MutexLocker locker(&mutex_);
...
//void SparsePoseGraph2D::AddSubmapFromProto(const int trajectory_id,
...
//void SparsePoseGraph2D::AddTrimmer(
...
void SparsePoseGraph2D::RunFinalOptimization() {
  WaitForAllComputations();
...
void SparsePoseGraph2D::RunOptimization() {
...
SparsePoseGraph2D::GetTrajectoryNodes() {
  common::MutexLocker locker(&mutex_);
  return trajectory_nodes_.data();
}

std::vector<SparsePoseGraph2D::Constraint> SparsePoseGraph2D::constraints() {
...
transform::Rigid3d SparsePoseGraph2D::GetLocalToGlobalTransform(
    const int trajectory_id) {
  common::MutexLocker locker(&mutex_);
...
std::vector<std::vector<int>> SparsePoseGraph2D::GetConnectedTrajectories() {

  return connected_components_.Components();
}

int SparsePoseGraph2D::num_submaps(const int trajectory_id) {
  common::MutexLocker locker(&mutex_);
  if (trajectory_id >= submap_data_.num_trajectories()) {
    return 0;
  }
...
mapping::SparsePoseGraph::SubmapData SparsePoseGraph2D::GetSubmapData(
    const mapping::SubmapId& submap_id) {
  common::MutexLocker locker(&mutex_);
...
SparsePoseGraph2D::GetAllSubmapData() {
  common::MutexLocker locker(&mutex_);
  std::vector<std::vector<mapping::SparsePoseGraph::SubmapData>>
...
transform::Rigid3d SparsePoseGraph2D::ComputeLocalToGlobalTransform(
...
mapping::SparsePoseGraph::SubmapData SparsePoseGraph2D::GetSubmapDataUnderLock(
    const mapping::SubmapId& submap_id) {
...
/*
SparsePoseGraph2D::TrimmingHandle::TrimmingHandle(SparsePoseGraph2D* const parent)
    : parent_(parent) {}

int SparsePoseGraph2D::TrimmingHandle::num_submaps(
    const int trajectory_id) const {
  return parent_->optimization_problem_.submap_data().at(trajectory_id).size();
}

void SparsePoseGraph2D::TrimmingHandle::MarkSubmapAsTrimmed(
    const mapping::SubmapId& submap_id) {
